...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        GT_rand = np.array(GT_rand).astype(np.float32)
        mylabel = np.array(mylabel).astype(np.float32)
        mylabel_binary = np.array(mylabel_binary).astype(np.float32)
        mylabel_rand = np.array(mylabel_rand).astype(np.float32)
        mylabel_rand_binary = np.array(mylabel_rand_binary).astype(np.float32)
...
class REFUGE_norm(object):
    def __call__(self, sample):
        ...
        mylabel_rand = np.ceil(np.abs(mylabel_rand - 255)/128.0).astype(np.float32)
        #mylabel_binary = np.ceil(np.abs(mylabel_binary - 255)/128.0).astype(np.float32)
        GT = np.ceil(np.abs(GT - 255)/128.0).astype(np.float32)
        GT_rand = np.ceil(np.abs(GT_rand - 255)/128.0).astype(np.float32)
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        GT_rand = np.array(GT_rand).astype(np.float32)#.transpose((2, 0, 1))
        mylabel = np.array(mylabel).astype(np.float32)#.transpose((2, 0, 1))
        mylabel_binary = np.array(mylabel_binary).astype(np.float32)#.transpose((2, 0, 1))
        mylabel_rand = np.array(mylabel_rand).astype(np.float32)#.transpose((2, 0, 1))
        mylabel_rand_binary = np.array(mylabel_rand_binary).astype(np.float32)#.transpose((2, 0, 1))
        mask = np.array(mask).astype(np.float32).transpose((2, 0, 1))

        img = torch.from_numpy(img).float()
        GT = torch.from_numpy(GT).float()
        GT_rand = torch.from_numpy(GT_rand).float()
        mylabel = torch.from_numpy(mylabel).float()
        mylabel_binary = torch.from_numpy(mylabel_binary).float()
        mylabel_rand = torch.from_numpy(mylabel_rand).float()
        mylabel_rand_binary = torch.from_numpy(mylabel_rand_binary).float()
...
class RandomHorizontalFlip(object):
    def __call__(self, sample):
        ...
        if random.random() < 0.5:
            ...
            GT_rand = GT_rand.transpose(Image.FLIP_LEFT_RIGHT)
            mylabel = mylabel.transpose(Image.FLIP_LEFT_RIGHT)
            mylabel_binary = mylabel_binary.transpose(Image.FLIP_LEFT_RIGHT)
            mylabel_rand = mylabel_rand.transpose(Image.FLIP_LEFT_RIGHT)
            mylabel_rand_binary = mylabel_rand_binary.transpose(Image.FLIP_LEFT_RIGHT)
...
class RandomVerticalFlip(object):
    def __call__(self, sample):
        ...
        if random.random() < 0.5:
            ...
            GT_rand = GT_rand.transpose(Image.FLIP_TOP_BOTTOM)
            mylabel = mylabel.transpose(Image.FLIP_TOP_BOTTOM)
            mylabel_binary = mylabel_binary.transpose(Image.FLIP_TOP_BOTTOM)
            mylabel_rand = mylabel_rand.transpose(Image.FLIP_TOP_BOTTOM)
            mylabel_rand_binary = mylabel_rand_binary.transpose(Image.FLIP_TOP_BOTTOM)
...
class RandomRotate(object):
    ...
    def __call__(self, sample):
        ...
        GT_rand = GT_rand.rotate(rotate_degree, Image.NEAREST)
        mylabel = mylabel.rotate(rotate_degree, Image.NEAREST)
        mylabel_binary = mylabel_binary.rotate(rotate_degree, Image.NEAREST)
        mylabel_rand = mylabel_rand.rotate(rotate_degree, Image.NEAREST)
        mylabel_rand_binary = mylabel_rand_binary.rotate(rotate_degree, Image.NEAREST)
...
class RandomScaleCrop(object):
    ...
    def __call__(self, sample):
        ...
        GT_rand = GT_rand.resize((ow, oh), Image.NEAREST)
        mylabel = mylabel.resize((ow, oh), Image.NEAREST)
        mylabel_binary = mylabel_binary.resize((ow, oh), Image.NEAREST)
        mylabel_rand = mylabel_rand.resize((ow, oh), Image.NEAREST)
        mylabel_rand_binary = mylabel_rand_binary.resize((ow, oh), Image.NEAREST)
        ...
        if short_size < self.crop_size:
            ...
            GT_rand = ImageOps.expand(GT_rand, border=(0, 0, padw, padh), fill=self.fill)
            ...
            mylabel_rand = ImageOps.expand(mylabel_rand, border=(0, 0, padw, padh), fill=self.fill)
            mylabel_rand_binary = ImageOps.expand(mylabel_rand_binary, border=(0, 0, padw, padh), fill=self.fill)
        ...
        GT_rand = GT_rand.crop((x1, y1, x1 + self.crop_size, y1 + self.crop_size))
        ...
        mylabel_rand = mylabel_rand.crop((x1, y1, x1 + self.crop_size, y1 + self.crop_size))
        mylabel_rand_binary = mylabel_rand_binary.crop((x1, y1, x1 + self.crop_size, y1 + self.crop_size))
...
class FixedResize(object):
    ...
    def __call__(self, sample):
        ...
        GT_rand = GT_rand.resize(self.size, Image.NEAREST)
        mylabel = mylabel.resize(self.size, Image.NEAREST)
        mylabel_binary = mylabel_binary.resize(self.size, Image.BILINEAR)
        mylabel_rand = mylabel_rand.resize(self.size, Image.NEAREST)
        mylabel_rand_binary = mylabel_rand_binary.resize(self.size, Image.BILINEAR)
